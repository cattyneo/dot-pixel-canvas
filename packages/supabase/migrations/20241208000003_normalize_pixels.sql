-- Migration: Normalize pixels input and allow trimmed hex
-- Description: Trims pixel strings before validation/insert and redefines exchange_art.
...
BEGIN
  ...
  -- Pixels validation: ensure array length then trim each value
  ...
  SELECT jsonb_agg(trim(both from value::TEXT))
  INTO normalized_pixels
  FROM jsonb_array_elements(new_pixels);

  FOR pixel IN SELECT value::TEXT FROM jsonb_array_elements(normalized_pixels) LOOP
