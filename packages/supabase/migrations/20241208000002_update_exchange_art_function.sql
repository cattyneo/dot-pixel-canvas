-- Migration: Update exchange_art RPC for Phase 1
-- Description: Adds validation, rate limiting, fingerprint/IP/work_seconds handling, and duplicate checks.

-- Clean up old signature (if any)
DROP FUNCTION IF EXISTS exchange_art(TEXT, JSONB);
...
BEGIN
  ...
  FOR pixel IN SELECT value::TEXT FROM jsonb_array_elements(new_pixels) LOOP
    IF pixel !~ '^#[0-9a-fA-F]{6}$' THEN
      RAISE EXCEPTION 'Invalid pixel format'
        USING ERRCODE = '22023';
    END IF;

    IF pixel NOT ILIKE '#ffffff' THEN
      is_all_white := FALSE;
    END IF;
  END LOOP;
  ...
  -- Duplicate pixel board check
  SELECT id, title, pixels, created_at
  INTO existing_post
  FROM posts
  WHERE pixels = new_pixels
  LIMIT 1;
  ...
  VALUES (sanitized_title, new_pixels, FALSE, client_fingerprint, client_ip, normalized_work)
